...
datamini <- merge(datamini[, 1:11], typicalSPI, by="interval")
inferSteps <- function(steps) {
    if (is.na(steps))  {
        stepInfer <- typSteps}
    else stepInfer <- steps
    }   

# find and replace NAs with our function.  Add result as a new column 
datamini$stepInf <- apply(datamini[,"steps"],1, inferSteps)
```
